...
def run_training(starting_epoch: int=0, ending_epoch: int=50):
    """
    Train the model by loading a already existing npy file containing all the training image data.

    This file is very similar to `model_training.ipynb` with only some
    changes to run just training and not create any graphs or evaluating the
    ...
    """
    print(f"Starting Epoch: {starting_epoch}, Ending Epoch: {ending_epoch}")
    ...
    if df_import.isnull().any(axis=1).sum() != 0:
        print("Exiting, there are nulls in the data.")
        return
    ...
    print("Full y_data:", y_output_data.shape)
    ...
    if not Path(TRAIN_IMAGES_ARRAY_NPY).exists():
        print(f".npy file not found at: {Path(TRAIN_IMAGES_ARRAY_NPY).absolute()}")
        print("Exiting")
        return
    print(" loading training images from npy file...")
    ...
    def r2_score(y_true, y_pred):
        """Custom R Squared metric as R2Score() in tensorflow 2.13.0 causes type error and is not available in earlier versions"""
    ...
    try:
        ...
        print("Start time:", start_training_time)
